...
The details of the translation are explained in Sec.~\ref{sec:v2c}. In summary, however, all the state-holding registers in the Verilog design have been gathered into a C struct. A function \texttt{top} is then defined that has the same parameters as the Verilog module it represents. When invoked, this function executes a sequence of state updates that simulate a single clock-cycle of hardware execution, according to the Verilog synthesis semantics. Because this is a sequentialization of the parallel register updates modelled by the Verilog, some shadow variables are introduced that record register values at the start of the clock cycle for subsequent use.  The non-synthesizable Verilog initial block is not included here, as this is a testbench component.  Placement of the assignment to \texttt{c} at the end of the body of \texttt{top} follows the `read after write' semantics for Verilog combinational logic. \rmcmt{The initial block in Software netlist is encoded in Figure 2. Note that Column 2 of Figure 2 contains the main() function that invokes top() as well as initializes state-holding elements. So, Figure 1 column 3 together with Figure 2 column 2 constitutes the whole software netlist along with property encoding. We need to clarify this in the text.}     
...
\rmcmt{can be moved to Column 1 of Figure 2} 
