...
by synthesizing the RTL design into two different netlist representations -- 
\emph{bit-level netlist} and \emph{software netlist}.  The bit-level 
netlist is expressed in AIGER format, whereas the software netlist 
is expressed in C language. 
%and is an \emph{exact} representation of the RTL design. 
%
Conventional tools for RTL verification typically reason over 
the bit-level netlist by translating them into CNF formula following 
...
inlcuding abstract interpretation and automata-theoretic 
trace abstraction techniques which was never applied to RTL verification. 
% 
To this end, we classify verification technologies on the bit-level netlist 
...
propositional SAT/SMT solvers which are used by techniques such as Bounded Model 
Checking (BMC), 
$k$-induction, Interpolation, IC3/PDR and others.  By contrast, abstraction-based tools 
use various convex and non-convex abstract domains which are used by techniques such as 
predicate abstraction and abstract interpretation. 
%
%present an emperical evaluation of various unbounded formal 
%verification algorithms such as Predicate Abstraction, $k$-induction, Interpolation, IC3/PDR 
%and Abstract Interpretation; 
This paper present the comparison of precise and abstraction-based verification tools 
from the Software Verification Competition (SV-COMP'16) and Hardware Model Checking Competition 
(HWMCC'16) for the purposes of RTL verification.  Experimental evaluation on a range of RTL 
benchmarks suggest that a commercial abstract interpretation tool Astr{\'e}e performs best among 
all software analyzers for verification of software netlist generated from the RTL.  On the other hand, 
the PDR engine of the well-known bit-level hardware model checking tool \textsc{ABC} performs best 
among the hardware tools. Furthermore, the performance of Astr{\'e}e is comparable to \textsc{ABC} 
