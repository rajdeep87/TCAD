\section{Property Checking Methods Investigated}\label{sec:techniques}

In this section, we briefly sketch the verification methods for property checking that
we explore in our experiments with conventional RTL verification tools and
native software analysis tools. These are all well-established techniques, 
documented in the literature---to which we refer the reader for in-depth explanation.
We broadly classify techniques into two classes, based on whether 
they use bit- or work-level decision procedures or employ abstract domains. We call these
\textit{SAT/SMT-based
Verification}, and \textit{Abstraction-based Verification}. 
%
For the purpose of illustration, we give bit-level descriptions of all 
the verification techniques.
%, though software verification algorithms work on program traces. 
