...
present a method for synthesis of a Verilog RTL design into an ANSI C program, which we
...
(blocking, non-blocking) and continuous assignments. Further, Verilog supports the 4-valued data types 
reg, wire, and integer.  All these constructs, combined with 
parallelism, make the translation challenging.

In view of these complexities, our translation is deliberately direct and transparent: we try to preserve the structure of the code, and refrain from optimizations or abstractions. Fidelity of the translated code to the semantics of the original Verilog is prioritised and we rely on the software analysis tools for efficiency. Fig.~\ref{ex1} shows an illustrative example.
...
%  Verilog & Synthesized Hardware & Software netlist \\
\hline
...
Figure~\ref{fig:steps} shows the translation steps of \emph{v2c}.  The
front-end phase performs Verilog parsing, macro preprocessing and
type-checking.  The Verilog front-end supports 1364-2005 IEEE Standard for
Verilog HDL.  The front-end generates a type-annotated
parse-tree which is passed to the translation phase. During translation phase,
the tool preserves the synthesis semantics and performs rule-based translation
following the Verilog module hierarchy. The rule-based translation produce vectored
assignments by mapping bit-operations to an equivalent shift and mask and
performs global dependency analysis to determine inter-module and intra-modular
...
The pseudo-code of the software 
