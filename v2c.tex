...
An example of inter-modular communication is illustrated using a D-flipflop
circuit in Figure~\ref{figure:design-granularity}.  The top-level 
module $\texttt{top}$ invokes the sub-module $\texttt{ff}$ which only 
models the sequential logic.  The equivalent translation in C (shown in right 
of figure~\ref{figure:design-granularity} preserves the modular structure. 
%
...
\hline
\end{tabular}
\caption{Inter-modular communication} 
\label{figure:design-granularity}
\end{figure} 
