...
The further values $X$ and $\const{Z}$ represent an unknown but binary logic level and an electrical
high impedance state, respectively.  The simplest synthesis semantics
for $\const{X}$ is to treate it as a don't-care assignment, which allows the
synthesis to choose a 0 or 1 to improve logic minimization.
V2C translates the value $\const{X}$ to a non-deterministic Boolean value in C.
The uncommon value $\const{Z}$ is outside the scope of our translation.
...
vectored expressions. Fig.~\ref{figure:bit} shows 
