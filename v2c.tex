\section{Verilog RTL to Software netlist}\label{sec:v2c}
%
...
dependencies.  The translation phase is followed by the code-generation phase 
where the intermediate vectored expressions and translated module items 
are converted to C-type expressions.  Note that we refrain from any optimizations
or abstractions to obtain a correct and trustworthy output. 

%
\begin{figure}[htbp]
\centering
\vspace*{0.3cm}
\scalebox{.50}{\import{figures/}{v2c.pspdftex}}
\caption{Translation stages in \emph{v2c} \label{fig:steps}}
\end{figure}
%
...
\emph{v2c} treat $X$ and $Z$ states to be non-deterministic as there is no
...
The dependency analysis routine determines all intra-modular dependencies
between the continuous assignments and always blocks. If a variable, say $x$,
appearing in continuous assignment is updated directly by the input signal and is 
read inside an always block, then it is placed before the always block to capture any 
update over the module input interface triggered from other modules or an 
external environment. Another possible scenario can be that a variable $x$ which is 
assigned in a continuous assignment statement appears in the right-hand 
side of another continuous assignment. In this case, the variable 
assignment $x$ is placed before the other assignment 
which reads $x$. An alternative scenario can be 
that the variable $x$ which appears in the right-hand side of a continuous assignment
is driven from an always block. This gives an ordering where the continuous 
assignment is placed after the always block to capture the updated value of $x$.  
All three cases are demonstrated with an example in Figure~\ref{figure:block}. 
For design with inter-modular 
combinatorial path or combinatorial loop, the output may settle after 
several executions before the next clock cycle. \emph{v2c} detects such 
combinatorial loop and flags a warning. The correct translation for 
designs with combinatorial exchanges between modules is to determine 
the stability condition for the combinatorial signals and 
execute the combinatorial logic until the stability condition is reached. 

\subsection{Bit-precise code generation}
\emph{v2c} generate bit-precise software-netlist model in C.
...
concatenation operators, reduction OR and other operators. \emph{v2c} retains 
...
%\emph{v2c} retains the word-level structure of the Verilog RTL. 
