...
The further values $X$ and $\const{Z}$ represent an unknown but binary logic level \rmcmt{not clear what this means ``but binary logic level"?} and an electrical
high impedance state, respectively.  The simplest synthesis semantics
for $\const{X}$ is to treate it as a don't-care; this allows synthesis to improve logic minimization.
...
the \texttt{integer} \tmcmt{Rajdeep: should ``integer'' be in tt font? Is this a Verilog keyword?} \rmcmt{Yes, I changed the font} declaration in Verilog, which is signed.
