...
in C language. 
\Omit{
%{\em v2c} follows synthesis semantics and not simulation semantics.  
The generated code is cycle-accurate as well as bit-precise. {\em v2c} 
...
\[ 
\begin{array}[t]{@{}lll}
\mathcal{SN} & {:}{:}{=} & \mathit{Mods} \\
\mathit{Mods} & {:}{:}{=} & \epsilon \mid \mathit{modName(V_1,\dots,V_n)} \{ Asgn \} \mathit{Mods} \\
\mathit{Asgn} &  {:}{:}{=} & \mathit{CAsgn} \mid \mathit{SAsgn} \mid \mathit{modName(V_1,\dots,V_n)}\\
\mathit{CAsgn} & {:}{:}{=} & (V_c := \mathit{Bv}) \mid (V_c := \mathit{Bool}) \\
\mathit{SAsgn} & {:}{:}{=} & (V_s := \mathit{Bv}) \mid (V_s := \mathit{Bool}) \\
...
\]
%
Here, $\mathit{Mods}$ are modules with names $\mathit{modName}$ and
input variables $V_1,\ldots,V_n \in \mathit{In}$. $\mathit{Asgn}$ is a
finite set of assignments partitioned into assignments
$\mathit{CAsgn}$, which define combinatorial variables $V_c$
(\emph{combinational/stateless} signals $\mathit{Comb}$ in the circuit
or ouput signals $\mathit{Out}$); and assignments $\mathit{SAsgn}$,
which assign \emph{sequential/state-holding} signals $V_s \in
\mathit{Seq}$ (registers).
\end{definition} 
%
A state transition in HW can be described by a set of register
updates, defined by the next-state function, and assignment of
non-deterministic values to external inputs. The state transition in
the software netlist corresponds to updates to the variables in
$\mathit{Seq}$ and explicit assignment of non-deterministic values to
the input variables $\mathit{In}$.
